...
appointment_schema.pre(
	'save',
	function (next) {
		const day =
			this.visit_date.getDay();
		if (day === 1 || day === 2) {
			throw new Error(
...
appointment_schema.pre(
	...
	function (next) {
		const minutes =
			this.visit_date.getMinutes();
		if (minutes !== 0) {
			console.log(
				'Minute appointment error'
			);
			throw new Error(
